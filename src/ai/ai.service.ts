...
export class AiService {
  ...
  async extractFromImage(
    ...
    for (const connector of this.connectors) {
      try {
        return await connector.extractFromImage(buffer, chatId);
      } catch (err) {
  ...
  async extractFromText(
    ...
    for (const connector of this.connectors) {
      try {
        return await connector.extractFromText(text, chatId);
      } catch (err) {
        lastError = err as Error;
        this.logger.warn(
  ...
  async classifyIntent(text: string, chatId?: string): Promise<string> {
    for (const connector of this.connectors) {
      try {
        return await connector.classifyIntent(text, chatId);
      } catch (err) {
        this.logger.warn(
  ...
  async transcribeAudio(buffer: Buffer, chatId?: string): Promise<string> {
    ...
    for (const connector of this.connectors) {
      try {
        return await connector.transcribeAudio(buffer, chatId);
      } catch (err) {
        lastError = err as Error;
        this.logger.warn(
