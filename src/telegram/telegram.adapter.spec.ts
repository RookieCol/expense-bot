...
describe('TelegramAdapter', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [TelegramAdapter, { provide: BOT, useValue: mockBot }],
    }).compile();
  ...
  it('sendText passes parseMode option', async () => {
    ...
    expect(mockSendMessage).toHaveBeenCalledWith(1, 'text', {
  ...
  it('sendMenu builds inline keyboard from sections', async () => {
    ...
    const keyboard = (opts.reply_markup as TelegramBot.InlineKeyboardMarkup)
      .inline_keyboard;
